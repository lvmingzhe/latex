...
## 1. 程序功能要点
语义引导的稠密重建程序主体部分以V1.6.2为基础，主要更新了以下三个文件：

1. 主程序: /main.cpp
   - 邻域图选择 bNeighborSelection 第1493行: SelectNeighborViews1(images, pointCloud); //语义导向的视角选择
   - 邻域图选择 camera_pose_neighbors.txt 第1495-1557行：保存了相机内外参数和邻域视图选择结果
   - 深度图计算 bDepthComputation 第1674行-1690行 : 设定局部地图重建范围，在sparse.ply中提取自己想设定的局部地图中心点
   - 深度图计算 bDepthComputation 第1752行：EstimateDepthMap1(images); //语义导向的深度图计算
   - 深度图过滤 bDepthFilter 第1933行：TriangulateFilterDepthMap(images[0]);   //语义导向深度图补全
2. 算法程序：/Algo/Algorithms.cpp
   - 第503行到514行，视角选择部分：可调整路面点，交通标识牌，其他点的语义权重系数
   - 第2250行 EstimateDepthMap1() 深度图计算可调整的地方有：
     - 深度图初始化三种方式：全随机/德劳内网格面片初始化/网格+语义调整初始化
       - 后边二者差别并不大
       - 其中网格初始化可以开启或关闭是否人为添加四个顶点的深度值 第1187行-第1275行(不添加深度图会缺失，添加会引入外点)
     - 自适应窗口放大倍数：线性关系放大/sigmoid函数关系放大/最小窗口/最大窗口
       - 前二者差别并不大，根据需要选择
     - 路面和交通标识牌平面RANSAC初始化: 可根据需要打开或全部关闭
     - 深度图低置信度得分过滤：滤除天空点 车辆 行人等动态物体，可根据需要开启或关闭
   - 第4018行 TriangulateFilterDepthMap()  深度图补全可以调整的地方有:
     - 深度过大的点跳过，面片顶点语义，面片顶点深度差异，面片顶点法向差异，面片大小
       - 外点过多时可采用严格的补全限制，反之完整性低时使用宽松的补全限制

3. 基本数据结构和函数声明：/base.h
